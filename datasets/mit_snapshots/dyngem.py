...
primary_model_build_epochs_number = 200  #0.86 in 200 100 # static 100 eppochs .75 static 200 epoch 0.81
dynamic_model_build_epochs_number = 100
...
def build_model():
    ...
    # Encoding Layer Dims
    encoding_dim = [48, 72 ,92, 93]
    # Decoding Layer Dims
    decoding_dim = [] 
    encoding_layers = []
    decoding_layers = []
    dynamic_series = loadRealGraphSeries('snapshots/s',1,8)
    count = 0
    node_count = []
    #Initial Values
    beta=2
    ...
    for g in dynamic_series:
        ...
        if count == 1 :
            ...
            model.fit(adj_mat,adj_mat,epochs=primary_model_build_epochs_number)
            ...
            if prev_N < N:
                ...
                curr_model.fit(adj_mat,adj_mat,epochs=dynamic_model_build_epochs_number)
